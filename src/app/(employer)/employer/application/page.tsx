...
function Application() {
  ...
  const {employerData,organizationData}:any =useContext(MyEmployerLogInContext);

  let limit: number = 4;
  const fetchResumeData = async (page: any,adminId:string|null) => {
    const resume = (await axios.post("/api/application/", { currentPage: page, limit: limit,employerId:employerData?._id})).data;
    setResumeData(resume?.data);
    setTotalPage(resume?.totalpage);
  }
  useEffect(() => {
    fetchResumeData(pagination,employerData?._id)
  }, [pagination]);
  const handlerStatus = async (text: any, id: any) => {
    // console.log(text,id);
    const send = (await axios.post("/api/application/status", { status: text, id: id,employerId:employerData?._id })).data;
    if (send) {
      setTimeout(() => {
        fetchResumeData(pagination,employerData?._id);
      }, 100);
  ...
  const handlerDeleteItem=async(id:any)=>{
    const send=(await axios.post("/api/application/delete",{id})).data;
    console.log(send);
    if(send.status==200){
      setTimeout(() => {
         fetchResumeData(pagination,employerData?._id);
