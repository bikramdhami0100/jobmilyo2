...
function UserPostJobLists() {
  ...
  return (
    <div>
       <Image className=' h-[40vh]  w-[40vw] flex items-center justify-center p-4 object-contain m-auto' src={"/images/fair.png"} height={100} width={100} alt='image'></Image>
       <h1 className=' self-center text-center text-3xl font-bold underline'>All posted jobs by user :</h1>
      {
                ...
                <div key={index} className={`  ${theme === "light" ? "bg-white" : ""} flex relative  flex-row  items-center justify-between gap-2 w-full h-full border p-4 mb-4 shadow-md `}>
                    ...
                    <div className='flex w-[30vw] flex-wrap  justify-center   items-center gap-2 '>
                      <Button className='w-[100px] bg-blue-600 flex gap-2 ' onClick={()=>{
                        router.push(`/user/userpostjobs/${item._id}`)
                      }}><Edit/> Edit</Button>
                       <Button className='w-[100px] bg-blue-600 flex gap-2 ' onClick={()=>{
